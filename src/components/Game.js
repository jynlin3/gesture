...
class Game extends React.Component {
	...
	GameServerRoomStart() {
		...
		function newRemoteFeed(id, display, audio, video) {
			...
			gestureGameroom.attach({
				...
				let subscribe = {request: "join", room: myroom, ptype: "subscriber", feed: id, private_id: mypvtid, };
				...
				ondata: function (data) {
					...
					if (json["textroom"] === "jointeam") {
						...
						if ('scoreA' in json){ scoreA += 1; } 
						else if ('scoreB' in json){ scoreB += 1; }
					}
				},
				oncleanup: function () {
	...
	handleJoinClick = (e) => {
		...
		updateTeamStatus();
		// send to remotes
		var message = { textroom: "jointeam", room: myroom, username: userName, team: teamId,};
		this.sendData(message);
	};
	...
	startGame = () => {
		...
		// notify remote players
			var message = { textroom: "startgame",room: myroom, };
			this.sendData(message);
		}
	...
	handleSubmit(event) {
		...
		if (correct) {
			...
			if (players.get(userName).team == 'A'){
				scoreA += 1;
				// send to remotes
				var message = { textroom: "answercorret", room: myroom, scoreA: scoreA, };
				this.sendData(message);
			} else {
				scoreB += 1;
				// send to remotes
				var message = { textroom: "answercorret", room: myroom, scoreB: scoreB, };
				this.sendData(message);        
			}
